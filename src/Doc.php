...
class Doc extends \DOTNET
{
     ...
     * are measured anti-clockwise with zero at three o'clock.The AddArc method returns the Object ID of the newly
     * added Graphic object.
     *
     ...
     */
    public function AddArc(float $AngleStart, float $AngleEnd, float $CenterX, float $CenterY, float $RadiusX, float $RadiusY, bool|null $Filled = null): int
    {
        return parent::AddArc($AngleStart, $AngleEnd, $CenterX, $CenterY, $RadiusX, $RadiusY, $Filled);
    }

    /**
     * Adds a bookmark which points to the current page.Bookmarks are specified as paths in much the same way as file
     * paths. Headings and subheadings are separated by a backslash character. For example:"1. Introduction\1.1 Aim and
     * Methods\1.1.3 Diagrams"When a bookmark is added, intermediate headings and subheadings will be created if they
     * do not already exist.To add multiple bookmarks all referring to the same page, simply call the AddBookmark
     * method multiple times.This method returns the Object ID of the newly added Bookmark object.
     *
     ...
     * the amount of spot colorant to use.There are two special color spaces:'All' refers collectively to all colorants
     * on an output device. It can be useful for printing registration marks.'None' indicates no colorant and will
     * never produce any visible output.
     *
     ...
     * to determine the best match.The following fonts are guaranteed to be available on every system.Additionally, you
     * can add any TrueType, OpenType or Type 1 font that you have installed on your system. The name you should use is
     * the one referenced in your fonts folder. For example.The AddFont method returns the Object ID of the newly added
     * Font object. Typically, you will want to assign this return value to the document Font property using code of
     * the form.theDoc.Font = theDoc.AddFont("Courier")If the specified font could not be found, then you will get an
     * Object ID of zero returned. You may wish to check for this possibility. Otherwise, a default font will be
     * used.Fonts are cached so newly added fonts will not be available to ABCpdf until the application is restarted.
     * If you need to dynamically load a font, you can pass this method a path to your font file. This will add the
     * font to the cache and make it available for use. You should not move, rename or delete a font file which has
     * been dynamically loaded using this technique.
     *
     ...
     * property of the Text object before adding a new item to the chain.This method returns the Object ID of the newly
     * added Text object. If no text could be added, then zero is returned. This will happen if a zero length string
     * was supplied or if the rectangle was too small for even one character to be displayed.Typically, you will get a
     * return value of zero if your text was too large to fit in your Rect or if the Pos was at the end of the
     * Rect.Text styles for the entire HTML content are determined at the point at which the first item in a text chain
     * is created. This means that varying document styles will not affect the way in which subsequent items in the
     * chain are displayed.For an example of chaining, see the Text Flow Example.Note that there is no requirement that
     * blocks be organized in a linear chain. That means that you can use the same ID as the ChainID parameter to
     * multiple calls to this method. If you wish, you can create trees of HTML blocks, flowing text from a chain head
     ...
     * the following methods to call dependent on the content of the ImageSpec.This method is provided for backward
     * compatibility. You should call the appropriate method directly rather than working via this method.
     *
     ...
     * assign a decode array using the ID returned from this method.To invert the transparency:theDoc.SetInfo
     * theDoc.GetInfo(theID, "XObject"),
     ...
     * - it doesn't offer any performance enhancements.Sometimes the IIS users don't have full access to the temp
     * directory. This is determined by the system setup you have on your machine. If this is the case, you will get
     * errors returned.So if you are working from ASP, you may find that you need to enable access to the temp
     * directory for the ASPNET user, the IUSR_MACHINENAME user or the IWAM_MACHINENAME user.Styles and Images. HTML
     * does not exist within a file, and so it does not have a location.External stylesheets and images are often
     * referenced via relative URLs. Because the HTML has no location, it is impossible to resolve these relative
     * reference.So you need to provide your stylesheet and image links as absolute references. Or you may be able to
     * use the HTML BASE element to specify an appropriate base location. Or you can save your HTML to file in an
     ...
     */
    public function AddImageHtml(string $Html, bool|null $Paged = null, int|null $Width = null, bool|null $DisableCache = null): int
    {
    ...
    /**
     * Some web pages are too large to fit on one PDF page.To split a web page across multiple PDF pages, you need to
     * call AddImageUrl or AddImageHtml to render the first page. The ID returned from these calls represents the head
     * of the chain.To add subsequent pages to the chain, you need to make calls to AddImageToChain passing in the
     * previous image from the chain each time.As well as using chaining to split web pages across multiple PDF pages,
     * you can also use it to split your web pages across multiple columns within a page. You can even split your chain
     * to generate multiple copies of the same page.More information can be found in the HTML
     ...
     * / CSS Rendering section of the documentation for details.Alternatively, it is possible that the PDF itself is
     * being cached. Most commonly, this can happen if you're streaming the PDF direct to the browser and you have
     * certain IIS settings
     ...
     * @return int The ID of the newly added View object.
     */
    public function AddImageUrl(string $Url, bool|null $Paged = null, int|null $Width = null, bool|null $DisableCache = null): int
    {
     ...
     * are represented, you should see the Adobe PDF Specification.Your newly added object needs to be referenced from
     * somewhere in the PDF document. If you do not reference your object, it will be orphaned and will be deleted when
     ...
     * document Page property using code of the form.theDoc.Page = theDoc.AddPage()Pages are added at the end of the
     * document. However, you can use the PageNum parameter to insert pages at other locations. The following code
     * inserts a page at the start of a document.theDoc.Page = theDoc.AddPage(1)Any existing page and all subsequent
     * pages will be shifted towards the end of the document to make room for the insertion.
     *
     ...
     * one page to another, you should use the AddHtml method, which is used for adding HTML styled text.The text is in
     * the current style, size and color and starts at the location specified in the current position. If the text is
     * long, it will will wrap and extend downwards until it fills the current rectangle. Text positioning in the
     * rectangle is determined by the horizontal and vertical positioning.The AddText method returns the Object ID of
     * the newly added Text object. If no text could be added, then zero is returned. This will happen if a zero length
     * string is supplied or if the rectangle is too small for even one character to be displayed.Typically, you will
     * get a return value of zero if your text was too large to fit in your Rect or if the Pos was at the end of the
     * Rect. So if you are expecting text to be displayed and are seeing a return value of zero, check your text size,
     * check your Rect is where you think it is by framing it using FrameRect, and ensure your Pos is set at the top
     * left of the Rect.Text is drawn word-wrapped within the current rectangle with the first character at the location specified by the Pos property. Normally, the Pos property reflects the top left position of the current rectangle. However, if you need to alter the position at which text drawing starts, you can modify the Pos property after changing the Rect. When the text has been drawn, the Pos will be updated to reflect the next text insertion point.Character positioning is specified from the top left of the character. The FontSize determines the total line height, and the character baseline is 80% of the way down from the top of the line.
     *
    ...
    /**
     * Use this method to append one PDF to the end of another one.Individual pages from one PDF can be drawn into
     * another using the AddImageDoc method.If you are inserting a number of pages, it is much faster to use the Append
     * method than to draw pages individually. It also has the advantage of maintaining other information such as
     * bookmarks.
     *
    ...
    /**
     * Use this method to determine whether an object is chainable.Some objects can be chained. A chunk of text may be
     * chained from page to page on the output PDF. Similarly, a web page may be chained from page to page.This method
     * allows you to determine whether the object you have added is chainable or whether it is at the end of the
     * chain.Only text, PostScript images and web pages can be chainable. So your ID should have been obtained from a
     * previous call to one of the AddImage family of calls or from AddHtml.
     *
    ...
    /**
     * Use this method to delete an object previously added to the document.Deletion is most commonly applied to pages
     * to remove them from the document. For example, to delete the current page, you might use the following
     * code:theDoc.Delete theDoc.Page
     *
     ...
     * system and that font substitutions will never need to be made.There are a number of reasons you may not wish to
     * embed fonts and, instead, use the AddFont method. Embedding fonts can increase the size of your PDF
     * considerably. Additionally, by distributing a PDF with an embedded font, you are actually redistributing the
     * font itself. You should check with your font supplier or legal department that you have permission to do
     * this.For information on how to specify font names, see the AddFont method.The EmbedFont method returns the
     * Object ID of the newly added Font object. Typically, you will want to assign this return value to the document
     * Font property using code of the form.theDoc.Font = theDoc.AddFont("Courier")If the specified font could not be
     * found, then you will get an Object ID of zero returned. You may wish to check for this possibility. Otherwise, a
     * default font will be used.Fonts are cached so newly added fonts will not be available to ABCpdf until the
     * application is restarted. If you need to dynamically load a font, you can pass this method a path to your font
     * file. This will add the font to the cache and make it available for use. You should not move, rename or delete a font file which has been dynamically loaded using this technique.
     *
     * @param string $FontName The name of the font typeface.
     * @param string|null $Language The language type to use (optional).
     * See the Fonts and
     ...
     * @return int The Object ID of the newly added Font object.
     */
    public function EmbedFont(string $FontName, string|null $Language = null, bool|null $Vertical = null, bool|null $Subset = null, bool|null $Force = null): int
    {
     ...
     * and any options are determined by the current options.By specifying values for the horizontal and vertical
     * radius parameters, you can draw rectangles with rounded corners. The values refer to the radii of the ellipse
     * used to draw the corners.By setting the horizontal radius parameter to half the width of the rect and the
     * vertical radius parameter to half the height of the rect, you can draw filled ovals and circles.The FillRect
     ...
     * and any options are determined by the current options.By specifying values for the horizontal and vertical
     * radius parameters, you can draw rectangles with rounded corners. The values refers to the radii of the ellipse
     * used to draw the corners.By setting the horizontal radius parameter to half the width of the rect and the
     * vertical radius parameter to half the height of the rect, you can draw ovals and circles.By default, frames are
     * drawn round the outside of the current rectangle. This allows you to add content and then frame it ensuring that
     * the frame and the content do not overlap. However, sometimes you may wish to draw the frame round the inside of
     * the rectangle. You can do this by setting the Inside parameter to true.The FrameRect method returns the Object
     ...
     * Similarly, you may wish to obtain raw data for insertion into a database.PDFs and Internet Explorer. The
     * combination of Internet Explorer and Acrobat is not always trouble-free - particularly under https or using
     * older versions of Internet Explorer. It can be difficult to know exactly where the problem lies because there is
     * an interaction of the Operating System, Internet Explorer and Acrobat. Any of these can be the cause.Sometimes,
     * Internet Explorer may get 'stuck' on a particular content type and insist on displaying your PDF as HTML. In
     * this case, you will see random text starting with %PDF. Sometimes, this can happen if you stream PDF data to a
     * window which was previously displaying HTML.Sometimes, server-side debugging results in extra data being
     * inserted into the content stream. While this may not matter for HTML, it will corrupt binary documents like
     * PDF.Sometimes, your code may inadvertently insert extra data into the content stream. Again, this will corrupt
     * the PDF. Error messages are a common cause of this kind of corruption.HTTP compression may result in PDF streams
     * being compressed before they are returned to the browser. While browsers are able to deal with this type of compression, Acrobat may not be able to. You can see whether this type of compression is being used by examining the headers returned to the browser using a tool like IEWatch. If you are using IIS 6 compression, you can disable it on a page-by-page basis using the IIS Metabase Explorer from the IIS 6 resource kit.For testing purposes, you may wish to change the content-disposition from 'inline' to 'attachment'. This will allow you to download the data rather than viewing it in your browser. You can then check the downloaded document using Acrobat or a hex editor.Alternatively, if the problem is that PDFs seem to be cached, you may wish to check the 'Enable Content Expiration' checkbox you will find under the Web Site Properties.We would suggest two steps:
     ...
     * This method allows you access to this information.There are core information types that all objects support.
     * There are also information types specific to particular types of object. You can use core information types to
     * allow you to iterate through every object in your document finding out specific information about each of
     * them.If the object does not exist or does not support the requested type of information, then an empty string is
     * returned.Every object supports the ID and Type properties. For more detailed information about this and other
     * properties, see the PDF Objects section of this document.PDF objects are case-sensitive, so be sure you use the
     * correct case when specifying information.
     *
     ...
     * should be combined but many situations are ambiguous.SVG is an XML based format for representing vector
     * graphics. Because SVG is standard XML, it's easy to parse and gives you the precise position of each item of
     * text on the page. The way that ABCpdf constructs the SVG should make it easy to extract any information you
     * require. ABCpdf currently supports SVG text and paths.For example, a simple "Hello World" PDF might produce the
     * following content:<?xml version="1.0" standalone="no"?>
     ...
     * and determine how objects in the PDF relate to objects in the SVG.For example, you could use SVG+ to identify
     * the section of a PDF stream that relates to a particular word on a page. You could then replace the text show
     * operator for that word with another one. Effectively, you'd be performing a low-level Search/Replace on the PDF
     * document.There is no official standard for SVG+, but if you are familiar with the PDF specification it should be
     * easy enough to understand.For example, a simple "Hello World" PDF might produce the following content:<?xml
     ...
     * Rect and MediaBox
     * properties will reflect the size of the first page in the document.ABCpdf operates an intelligent just-in-time
     * object loading scheme, which ensures that only those objects that are required are loaded into memory. This
     * means that if you are modifying large documents, then server load will be kept to a minimum. The original PDF
     * document must be available for as long as the Document object is being used.As a result, you cannot overwrite a
     * PDF file while it is open for reading. You will need to save your PDF to another location and then swap the two
     * files round.Object deletion requires that all references to an object be removed. There is no way of doing this
     * without checking each object in the document. So object deletion requires that every object in the document is
     * loaded, and for large documents, this may place a significant load on the server.Reading encrypted documents
     * places a greater load on the server because - like object deletion - it requires that every object in the
     * document be loaded.Please note that you are legally bound to respect the permissions present in existing PDF
     * documents. For details, please see the Legal Requirement Section.The Read method may be used to read eForm FDF documents as well as PDF documents. Most PDF operations will not work on FDF documents, but you can query field values using the GetInfo methods to return Unicode strings.Modifying Documents. ABCpdf will allow you to open, modify and save PDF documents.ABCpdf will allow you to draw on top of PDF documents or add or delete pages or modify document data. However, because of the way that PDF documents are structured, it's unlikely that you'll be able to edit existing content.So if there are blank spaces which you can draw your entries into, that will work. Indeed, you might want to draw a white box over existing content and then draw on that.However, you shouldn't expect to be able to edit and re-flow text that is already in your PDF.
     *
     ...
     * a document. It can be used for reordering, copying or deleting pages.It accepts a list of page numbers and
     * reorders the pages in the document so that they mach these page numbers. If a number is repeated more than once,
     * the page is duplicated. If a number is omitted, the page is deleted.Page numbers can be delimited using spaces,
     * commas or semicolons. The first page in a document is page one. So to reverse a four-page document, you might
     * use "4 3 2 1".
     *
     ...
     * path is used as the destination. Any existing file will be overwritten.ABCpdf operates an intelligent
     * just-in-time object loading scheme, which ensures that only those objects that are required are loaded into
     * memory. This means that if you are modifying large documents, then server load will be kept to a minimum. The
     * original PDF document must be available for as long as the Document object is being used.As a result, you cannot
     * overwrite a PDF file while it is open for reading. You will need to save your PDF to another location and then
     * swap the two files round.If you need to obtain a PDF as raw data, you can use the GetData method.Versions.
     * ABCpdf automatically determines the PDF version depending on the features you use. If you use features from only
     * the 1.1 specification, it will write a 1.1 PDF. If you use 1.3 features, it will write a 1.3 PDF. If you use 1.4
     * features, it will write a 1.4 PDF. Ditto 1.5 and 1.6.If you're using a PDF template or drawing from another PDF,
     * the final output will be the minimum version used in these templates. In many real world applications, this will
     * be the factor which determines the version in the final output produced by ABCpdf.There is no advantage in producing a 1.6 document if you're not using features from the 1.6 feature set. To do this will simply stop users of older versions of Acrobat from accessing a document which should be available to them.
     *
