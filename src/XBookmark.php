...
class XBookmark extends DOTNET
{
    ...
    /**
     * Use this method to move bookmarks within the bookmark hierarchy.The bookmark to be adopted is detached from the bookmark hierarchy.
     * Then it is added at the end of the bookmark collection for the current
     * object.Some Adopt operations are not possible. For example, a child cannot
     * adopt its parent. In these cases, the field structure will be left
     * unchanged. 
    ...
    /**
     * Use this method to insert a new bookmark at a specified location.A new bookmark is created and given the title specified. It is
     * added at the specified location, and the newly created bookmark is returned.If the index is greater than or equal to the number of items in
     * the collection, then the bookmark is appended to the end.Bookmarks that follow the insertion point move down to accommodate
     * the new bookmark. 
    ...
    /**
     * Use this method to refresh and reload all children of this bookmark.When a bookmark is first referenced, the bookmark data from the
     * PDF is cached. This allows a level of optimization which would not
     * otherwise be possible.However, if you are using the low-level functionality to modify
     * the bookmark structure, the cache will not reflect your changes.
     * In this situation, you can force the bookmarks to be reloaded by
     * calling Refresh. 
     */
    ...
    /**
     * Use this method to remove the bookmark at the specified location.The bookmarks that follow the removed bookmark move up to occupy
     * the empty spot. 
