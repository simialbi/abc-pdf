...
class XSignature extends DOTNET
{
     ...
     * document.Normally, this is done when you save the document. It happens
     * invisibly without you needing to do anything. However, sometimes, you
     * may wish to commit before the document is saved. This most commonly
     * occurs if you are working with documents containing more than one
     * signature field.The PDF architecture requires that a document be incrementally
    ...
    /**
     * Use this method to extract the encoded X.509 data of the
     * certificate that was used to sign the document. Normally, there is
     * only one certificate returned, but for some documents, you may receive
     * additional certificates that can be used to create a certificate
     * chain. In such cases, the first certificate is always the certificate
     * that was used for signing.You can pass the data returned by this function to the
     ...
     * for details.The certificates you provide will be cached at
     ...
     * there is no certificate to validate against.ABCpdf does not currently do revocation checks
     ...
     * GetCertificates
     * method and check the certificates yourself.If a certificate is unavailable or invalid,
     * this method may raise an error. This means
     * validating against an unsigned signature field
     * will cause an error to be raised.How does Adobe Reader validate a
     * PDF document without certificate
     * files?You may find that Adobe Reader does
     ...
     * certify PDF documents:In order to validate a PDF document
